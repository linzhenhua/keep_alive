                ...
                while(1)
                {
                    ...
                    if(recv_res < 0)
                    {
                        ...
                        if( (errno == EAGAIN) || (errno == EWOULDBLOCK) )
                        {
                            ...
                            char hex[1024] = {'\0'};
                            EbcToAsc(client_recv_buf, hex, recv_len);
                            DaemonLog(0, hex);
                            ...
                            {
                                /*把数据发送到农行的客户端conn_fd*/
                                int send_res = send(conn_fd, hex, recv_len*2, 0);
                                if( send_res < 0 )
                                {
                ...
                while(1)
                {
                    ...
                    if(recv_res < 0)
                    {
                        ...
                        if( (errno == EAGAIN) || (errno == EWOULDBLOCK) )
                        {
                            ...
                            AscToEbc(server_recv_buf, recv_len);

                            send_res = send(g_client_fd, server_recv_buf, len, 0);
                            if( send_res < 0 )
                            {
